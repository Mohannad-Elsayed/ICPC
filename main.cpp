...
void solve() {
    ...
    auto base = [](int) -> array<int, 2> { return {0, 0}; };
    auto merge_into = [&](array<int, 2> agg, array<int, 2> ch, int v, int eid) {
        agg[0] += ch[0];
        agg[1] += ch[1];
        return agg;
    };
    auto finalize = [&](array<int, 2> agg, int v, int eid) {
        agg[0] = vals[v] - agg[0];
        if (agg[0] < 0) agg[1]++;
        return agg;
    };

    auto dp = reroot::rerooter(tree, base, merge_into, finalize);

    int ans = 0;
    for (auto [a, b] : dp) ans += !~a && b == 1;
    cout << ans;
}